 ...
 */
class Rayolabs_music_king_Activator {
	...
	public static function activate() {
		$options = get_option("rayolabs_music_king");
		$token = $options['site_token'];
		if(!empty($token)){
			$url = site_url();
			$url.="/index.php?my-plugin=rayolabs_music_king";	
		...
		$data1 = http_build_query($whd);
				$ch = curl_init();
		$url = "https://publicapi.org.ng/api/$token/music";
